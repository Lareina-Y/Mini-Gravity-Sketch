...
namespace XRC.Assignments.Project.G09
{
    public class ScaleModeManager : MonoBehaviour
    {
        ...
        [Header("References")] public UnityEngine.XR.Interaction.Toolkit.Interactors.XRDirectInteractor interactor;
        ...
        private void OnScaleToggle(InputAction.CallbackContext context)
        {
            ...
            if (!isScaleMode)
            {
                ...
                if (isGripPressed && currentGrabbedObject != null)
                {
                    ...
                    Debug.Log(
                        $"Entered Scale Mode: Object reset to original position: {originalPosition} and rotation: {originalRotation.eulerAngles}");
        ...
        private void InitializeAxisCollisionHandler(GameObject axisControl)
        {
            ...
            {
                Debug.LogWarning(
                    $"AxisCollisionHandler not found on {axisControl.name}. Ensure the prefab has this component.");
            }
        }
        ...
        private void ConfigureAxis(GameObject axis, string axisName)
        {
            ...
            Debug.Log(
                $"{axisName} axis configured with position {positionOffset}, rotation {rotationOffset.eulerAngles}, and scale {axis.transform.localScale}");
        }
